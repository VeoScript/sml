        ...
        <div className="flex flex-col w-full py-5 space-y-1">
          {navigations.map(({ name, icon, href }, i) => (
            <Link href={href} key={i}>
              <a className={`${router.pathname === href ? 'bg-marigold text-white hover:bg-opacity-80' : ' text-opacity-80 bg-none hover:bg-marigold hover:bg-opacity-30'} flex flex-row items-center text-lg text-panther rounded-full space-x-3 px-5 py-2 transition ease-in-out duration-300`}>
                <span>{icon}</span>
                <span>{name}</span>
              </a>
            </Link>
          ))}
          <button onClick={handleLogout} className="flex flex-row items-center text-lg text-panther text-opacity-80 rounded-full space-x-3 px-5 py-2 transition ease-in-out duration-300 bg-none hover:bg-marigold hover:bg-opacity-30">
